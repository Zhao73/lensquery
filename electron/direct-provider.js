...
const MAX_TEXT_EVIDENCE_CHARS = 90_000
...
export async function runDirectProvider({ profile, secret, request, settings, fetchImpl = globalThis.fetch, readFile = fs.readFile }) {
  ...
  const payload = profile.kind === 'anthropic'
    ? anthropicPayload(profile, prompt, conversation, images)
...
function anthropicPayload(profile, prompt, conversation, images) {
  const messages = conversation.map((message) => ({
    role: message.role,
    content: message.content,
  }))
  ...
  messages.push({ role: 'user', content })
  return { model: profile.model, max_tokens: 4096, messages }
}
...
async function collectEvidence(request, readFile) {
  ...
  for (const file of request.files || []) {
    ...
    if (file.videoPreparation?.frames) {
      for (const frame of file.videoPreparation.frames) {
