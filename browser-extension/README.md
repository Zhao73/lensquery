...
- one explicit **使用 LensQuery 分析当前网址** action on the webpage background and the extension-icon menu;
- one-click current-page analysis from the LensQuery toolbar icon, plus the `lq` address-bar keyword for a pasted or typed URL;
...
5. Reload the extension once, then start LensQuery. You can now:
   ...
   - when a site suppresses Chrome's native menu, choose the compact **使用 LensQuery 识别** action shown next to the site's own menu;
   - press `Ctrl+Shift+Space` and use the two-click DOM picker; the second click submits immediately without a prompt composer.
   - click the LensQuery toolbar icon to analyze the complete current page directly;
