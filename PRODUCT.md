...
- Video analysis is provider-independent: LensQuery locally probes the file, samples bounded time-coded frames, extracts a compact mono audio track, and reads bounded same-name VTT/SRT subtitles when present. Current CLI routes label subtitle coverage and any remaining audio as untranscribed instead of inventing speech; the browser connector may fetch only caption tracks already published by the active YouTube page or transcript segments already exposed by a generic page.
