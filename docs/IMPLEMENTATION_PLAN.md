...
- Images and supported video containers are checked automatically with the official C2PA Rust SDK plus a release-pinned official trust-list snapshot before model analysis starts. Exact source types, common EXIF/container fields, bounded image forensic derivatives, and visual inference remain separate. Trusted C2PA prompt ingredients and common PNG generation metadata are extracted into separate verified/untrusted prompt tiers; missing prompt strings fall back only to labeled reconstruction. PDF/text/browser selections receive the same strict provenance boundary, with provider-keyed text watermark detectors required for a verified result. Browser evidence includes a bounded hidden-DOM audit with prompt-injection labeling.
