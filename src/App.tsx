...
function SettingsPanel(props: { settings: AppSettings; onSave: (settings: AppSettings) => Promise<void> }) {
  ...
  const screenPermissionLabel = permissions?.screenCapture
    ...
    : permissions?.screenCaptureRestartRequired
      ? '正在自动重启'
      : '需要开启'
  const electronPermissionName = permissions?.applicationName || 'LensQuery Electron Preview'
  const electronPermissionPath = permissions?.applicationPath || '/Applications/LensQuery Electron Preview.app'
  return (
